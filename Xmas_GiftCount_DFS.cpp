...
//            routeMap[connector_house] = v;
            ...
            sort(directlyConnectedHouses.begin(), directlyConnectedHouses.end());
//        }
        singlyConnected = (directlyConnectedHouses.size() == 1);
    }
...
void findNextHouses(int start_h, int end_h, House* h_arr[])
{
    ...
    vec_int connectedHouses = h_arr[start_h-1]->getMapKeys();

    for(int i=0;i<connectedHouses.size();i++)
    {
        if (!h_arr[connectedHouses[i]-1]->isSinglyConnected())
        {
            st.push(connectedHouses[i]);
        }
    }
